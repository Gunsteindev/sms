...
import { buildDashboardModel, type Stats, type TrendPoint, type Secondary, type DashboardModel } from '@/lib/dashboard-model';
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function DashboardClient({ initial }: { initial: DashboardModel }) {
    const [stats,       setStats]       = useState<Stats | null>(initial.stats);
    const [secondary,   setSecondary]   = useState<Secondary | null>(initial.secondary);
    const [trends,      setTrends]      = useState<TrendPoint[]>(initial.trends);
    const [currentYear, setCurrentYear] = useState<AcademicYear | null>(initial.currentYear);
    const [currentTerm, setCurrentTerm] = useState<Term | null>(initial.currentTerm);
    const [openSession, setOpenSession] = useState<PoolSession | null>(initial.openSession);
    const [poolRevenue, setPoolRevenue] = useState(initial.poolRevenue);
    const [loading,     setLoading]     = useState(false);
    const [error,       setError]       = useState(false);

    const applyModel = (m: DashboardModel) => {
        setStats(m.stats);
        setSecondary(m.secondary);
        setTrends(m.trends);
        setCurrentYear(m.currentYear);
        setCurrentTerm(m.currentTerm);
        setOpenSession(m.openSession);
        setPoolRevenue(m.poolRevenue);
    };

    // Initial data is server-rendered (see dashboard/page.tsx). load() backs the
    // Refresh button, re-fetching through the API and rebuilding via the same
    // shared model the server used.
    const load = async () => {
        ...
        try {
            ...
            applyModel(buildDashboardModel({
                full:         ok(fullRes)?.data ?? null,
                years:        ok(ayRes)?.data   ?? [],
                terms:        ok(termRes)?.data ?? [],
                departments:  ok(deptRes)?.data ?? [],
                gradeLevels:  ok(glRes)?.data   ?? [],
                scholarships: ok(schRes)?.data  ?? [],
                sessions:     ok(poolRes)?.data ?? [],
                vehicles:     ok(transRes)?.data ?? [],
                activities:   ok(actRes)?.data  ?? [],
                announcements: ok(annRes)?.data ?? [],
            }));
        } catch {
            setError(true);
            toast.error('Failed to load dashboard data');
