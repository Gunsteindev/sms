...
// Server-fetched initial payload (see reports/page.tsx) for the default 30-day range.
export interface ReportsInitial {
...
// ─── Main page ────────────────────────────────────────────────────────────────
export default function ReportsClient({ initial }: { initial: ReportsInitial }) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [classes,      setClasses]      = useState<any[]>(initial.classes);
  const [feePayments,  setFeePayments]  = useState<FeePayment[]>(initial.feePayments);
  const [poolSessions, setPoolSessions] = useState<PoolSession[]>(initial.poolSessions);
  ...
  // Initial 30-day data is server-rendered; only refetch when the range changes.
