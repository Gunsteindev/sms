...
async function main() {
    ...
    console.log('Fetching raw row from sms_employees (no $select)...\n');
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const r = await axios.get<any>(`${API}/sms_employees?$top=1`, { headers: h, timeout: 20000 });
        const rows = r.data.value ?? [];
        if (!rows.length) {
            console.log('Table is empty — fetching metadata from EntityDefinitions...');
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const meta = await axios.get<any>(
                `${API}/EntityDefinitions(LogicalName='sms_employee')/Attributes?$select=LogicalName,DisplayName,AttributeType&$filter=AttributeType ne 'Virtual'&$orderby=LogicalName`,
            ...
            attrs.forEach((a: any) => {
                ...
                console.log(`  ${a.LogicalName.padEnd(45)} [${a.AttributeType}]  "${display}"`);
            });
        } else {
