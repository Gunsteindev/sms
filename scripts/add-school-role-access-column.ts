/**
 * add-school-role-access-column.ts
 *
 * Adds sms_rolemoduleaccess (Memo / multiline text) to the sms_schools entity.
 * It stores a JSON map of { roleNumber: moduleKey[] } so the super admin can set
 * per-role module access from the User Management → Module Access tab.
 * Read by mapSchool / written by buildSchoolPayload in src/lib/dataverse/school.ts.
 *
 * Run: npx ts-node --skipProject scripts/add-school-role-access-column.ts
 */
...
const memoAttr = (schemaName: string, displayName: string, maxLength: number) => ({
...
async function addColumnIfMissing(client: AxiosInstance, logicalName: string, attr: ReturnType<typeof memoAttr>): Promise<void> {
    try {
        await client.get(
...
async function main() {
    ...
    await addColumnIfMissing(client, logicalName, memoAttr('sms_rolemoduleaccess', 'Role Module Access', 10000));
    ...
    const probe = await client.get(`/sms_schools?$select=sms_schoolid,sms_name,sms_rolemoduleaccess&$top=1`);
