/**
 * create-sms-programmetracks-table.ts
 *
 * Migrate Programme Tracks from localStorage-only (SchoolSettingsContext) to a
 * Dataverse-backed entity, matching the schema to be coded in
 * src/lib/dataverse/programmetracks.ts:
 *   sms_name (primary), sms_abbreviation (string, max 12), sms_description (memo),
 *   sms_color (string), sms_ordernumber (integer)
 * plus lookup: sms_school.
 *
 * Run: npx ts-node --skipProject scripts/create-sms-programmetracks-table.ts
 */
...
async function main() {
    ...
    console.log('\n══════════════════════════════════════════════════');
    console.log('  Create Dataverse table: sms_programmetrack');
    console.log('══════════════════════════════════════════════════\n');
    ...
    console.log('1. Creating entity sms_programmetrack with attributes');
    try {
        await client.post('EntityDefinitions', {
            '@odata.type': 'Microsoft.Dynamics.CRM.EntityMetadata',
            SchemaName: 'sms_programmetrack',
            DisplayName: label('Programme Track'),
            DisplayCollectionName: label('Programme Tracks'),
            Description: label('Academic programme tracks (e.g. General Science, Business, Visual Arts) offered by the school.'),
    ...
    const ent = await client.get(`EntityDefinitions(LogicalName='sms_programmetrack')?$select=LogicalName,EntitySetName,MetadataId`);
    ...
    const lookups: { schema: string; refEntity: string; refAttr: string; lookupSchema: string; label: string }[] = [
        { schema: 'sms_programmetrack_sms_school', refEntity: 'sms_school', refAttr: 'sms_schoolid', lookupSchema: 'sms_school', label: 'School' },
    ];
    ...
    for (const lk of lookups) {
        try {
            await client.post('RelationshipDefinitions', {
                ...
                ReferencingEntity: 'sms_programmetrack',
    ...
    try {
        await client.post('PublishXml', {
            ParameterXml: '<importexportxml><entities><entity>sms_programmetrack</entity></entities></importexportxml>',
        });
