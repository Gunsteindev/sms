...
async function main(){
  ...
  const r=await axios.get<any>(`${API}/EntityDefinitions(LogicalName='sms_announcement')/Attributes?$filter=IsCustomAttribute eq true&$select=LogicalName,AttributeType`,{headers:h,timeout:30000});
  r.data.value.sort((a:any,b:any)=>a.LogicalName.localeCompare(b.LogicalName)).forEach((a:any)=>console.log(`  ${a.LogicalName.padEnd(40)} ${a.AttributeType}`));
}
