...
async function main() {
    ...
    const r = await axios.get<any>(`${API}/EntityDefinitions(LogicalName='sms_user')/Attributes?$filter=IsCustomAttribute eq true&$select=LogicalName,AttributeType,RequiredLevel`, { headers: h, timeout: 30000 });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    r.data.value.sort((a: any, b: any) => a.LogicalName.localeCompare(b.LogicalName)).forEach((a: any) => console.log(`  ${a.LogicalName.padEnd(40)} ${a.AttributeType.padEnd(15)} ${a.RequiredLevel?.Value ?? ''}`));
}
