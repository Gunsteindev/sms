...
async function main() {
    ...
    console.log('Fetching raw row from sms_vehiclemaintenances (no $select)...\n');
    try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const r = await axios.get<any>(`${API}/sms_vehiclemaintenances?$top=1`, { headers: h, timeout: 20000 });
        const rows = r.data.value ?? [];
        if (!rows.length) {
            console.log('Table is empty — fetching metadata...');
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const meta = await axios.get<any>(
                `${API}/EntityDefinitions(LogicalName='sms_vehiclemaintenance')/Attributes?$select=LogicalName,DisplayName,AttributeType&$filter=AttributeType ne 'Virtual'&$orderby=LogicalName`,
